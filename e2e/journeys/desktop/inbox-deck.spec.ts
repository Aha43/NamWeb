...
test('the deck cycles: skipped items come around again until resolved', async ({ page, doc }) => {
  ...
  await expect(dialog.getByText(/First thought · 3 left/)).toBeVisible();

  // Skip past the end — the deck wraps instead of slamming shut, and the count stays.
  await dialog.getByRole('button', { name: /Skip/ }).click();
  await dialog.getByRole('button', { name: /Skip/ }).click();
  await expect(dialog.getByText(/Third thought · 3 left/)).toBeVisible();
  ...
  await expect(dialog.getByText(/Second thought · 2 left/)).toBeVisible();
  ...
  await expect(dialog.getByText(/Third thought · 1 left/)).toBeVisible();
  await dialog.getByRole('button', { name: 'It’s one action' }).click();
...
test('the deck honors the selection: Process selected walks only the ticked items', async ({ page, doc }) => {
  ...
  const dialog = page.getByRole('dialog', { name: 'Process inbox' });
  await expect(dialog.getByText(/First thought · 2 left/)).toBeVisible();
  await dialog.getByRole('button', { name: 'It’s one action' }).click();
  await dialog.getByRole('button', { name: 'Do it next' }).click();
