...
- **Counter resources.** A new resource type: give an action a counter ("boxes to the attic · 0/12") and tick it up right from the list — the +1 pill saves immediately, no editor, no Save, and fills green at the target (it signals; finishing the action stays your call). Behind it, resource types moved onto a small registry — the next type is a one-entry job — and the types finally wear names instead of shouting URI/FILE at you. Closes #798.
