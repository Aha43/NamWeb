...
export class PostgresAuthStore implements AuthStore {
  ...
  async claimRefresh(id: string, expectedGeneration: number, leaseSeconds: number, lockId: string): Promise<boolean> {
    ...
    // lock lives in the (unencrypted) `data` JSONB beside the generation. rowCount 0 = not claimed.
  ...
  async finalizeRefresh(id: string, lockId: string): Promise<number | null> {
    // Bump the generation (superseding the old token), drop the lock, and slide the expiry (#1053) — but
    // ONLY while the live lock is still ours (`lockId` matches). Called AFTER the new refresh token is
    // durably issued, so the generation never runs ahead of a usable replacement. Zero rows = the lock is
    // no longer ours (lease expired + reclaimed by a newer request), so we must NOT advance the generation.
    const { rows } = await this.pool.query<{ gen: number }>(
      ...
      [id, lockId, nowSeconds() + grantTtlSeconds()],
    );
