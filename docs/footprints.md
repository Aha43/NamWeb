...
*(the "checklists" minor — one feature built as four sequenced PRs off a spec that had itself been the data lost-and-recovered in the #1141 incident; core + Reset + checkbox rendering + single-toggle filter; inline pre-cut review clean; minor)* **Checklists, by reinterpreting the tree rather than extending it.** The whole cut is one feature — a project tagged **`#checklist`** becomes a checklist — and the design's insistence is that it's *not a new data model*: a checklist is a project you read differently, so it reuses ordering, completion, traversal, and the summary export for free. The arc came out of real use in a way worth noting: the spec was **the 4,000-char document orphaned and recovered in the #1141 data-loss incident**, and it drove four sequenced PRs. **Core (#1147):** a project-scoped system tag whose check-items are suppressed from every day-to-day surface (Next/Backlog/Due/Calendar/Blocked/context/Loose-ends) via the same `subtreeIdsWhere` machinery SOMEDAY generalised — but with a deliberate twist (`checklistSuppressedIds` excludes the project node itself, so the checklist stays visible while its items drop out), and the checklist is never flagged stalled. Its one hard **invariant — a checklist holds check-items but never sub-projects — is enforced loudly**, not by silent reshape: a shared `validateIntent` throws in the MCP commit wrapper and toasts on the web, with an `applyIntent` no-op backstop keeping replay safe, and the move-target lenses simply never offer a checklist as a project destination. That "fail loudly, never silently reinterpret" rule is the spec's own words — the document that argued for it is the one #1141 nearly ate. **Reset (#1149):** one button flips every done check-item back to Backlog for the next run — the recurring-template win *without a recurrence engine* (the YAGNI call from #613 vindicated again). **Rendering (#1153):** check-items show a **checkbox** in place of the status dropdown (check→Done, uncheck→Backlog), trailing so it never collides with the bulk-select box; new items are born unchecked; the sub-projects add-row disappears; bulk ops drop "make sub-project" and reframe status to Mark done / not done; reorder and move-out (the promotion path) stay. **Filter (#1155):** three status boxes collapse to a single **Show done** toggle, since a checklist only has two states. **Converging — on the model, and closing a loop.** Not the AI arc; this is the GTD model absorbing checklists the orthodox way (Allen endorses them — packing lists to the weekly review), and it quietly retires ~a third of the `#not-stalled` workaround (its recurring-checklist carriers now have a real home). The build was **spec-driven and incremental** — four small green PRs, each merged after a live preview test, the last two folded via a stacked-then-rebased branch when a base-delete auto-closed the PR. Banked: a new *kind* of thing is often best expressed as a **reinterpretation of the existing tree** (no new model = no new sync contract, no duplicated traversal); a structural invariant belongs in a **shared validator called by every write surface** (MCP throw + web toast + reducer no-op backstop) so it can't be bypassed by the client doing the work; and this i18n catalog uses **single-brace `{x}`** interpolation, not i18next's default `{{x}}` (the latter renders literally and broke an aria-label test until caught). **Next:** redeploy the MCP server so the connector learns `#checklist` and enforces the invariant (currently web-only); the parked **SOMEDAY/checklist dated-on-calendar** question (a due date arguably should still show on the calendar under either); and the low-fruit **time-aware overdue** (#1152).

## v3.5.1 — 2026-08-18

*(a single data-loss fix cut off-cycle in response to a real incident — reorder-replay aliasing, closed with reconcile + detach-all + ingest-level self-heal; 1 fix PR, a Codex P1 round, and an orphan recovered live; patch)* **A quiet concurrency bug that could lose work, closed — and the workspace learned to self-heal.** The first off-cycle patch in the 3.x line, and it came from **use, not planning**: after a week of daily reordering, the author hit a bug where two actions in a sprint project vanished and one from a sibling appeared under *both* — the signature of a node aliased into two `childIds`. The diagnosis ran **live over the MCP connector** (read-only sweep of the real workspace found exactly one corruption site: a project with `childCount 2` but zero resolvable children — dangling refs), and the root cause was a **reorder colliding with a concurrent writer** on the single JSONB doc: on a version conflict `commitIntent` **replays** the reorder, and `reorderChildren` *overwrote* the parent's `childIds` verbatim, resurrecting an id a concurrent move had taken out (aliasing it) while `detach()` — first-parent-only — left a dangling ref on delete. The fix is three-layered: `reorderChildren` now **reconciles** against current children instead of overwriting (never resurrects/drops on replay), `detach()` clears **every** parent, and a new `normalizeChildIds` runs at the **single remote-ingest choke point (`pull`)** so no synced snapshot — web load, Realtime reconcile, conflict-replay, or MCP `loadSnapshot` — can carry the corruption forward. The **Codex gate earned its keep on exactly its class again**: it passed the reconcile/detach logic but flagged a **P1** — the first cut healed only `load()`, leaving Realtime and conflict-replay bases un-normalized (and the "repair rides the next write" claim was false, since replay builds on the freshly-pulled unnormalized doc) — which is what drove the fix down to `pull`. The tail was the most instructive part: asked to reproduce the loss, the **MCP assistant recovered the "lost" data** — it was **orphaned, not deleted** (still in the doc, just unreachable), restored by `move_node` with the id it still held. That reframed the whole incident: *the failure orphans rather than deletes*, and an orphaned node is indistinguishable from a deleted one in the UI — so silent casualties from before may still be sitting recoverable in the doc. Two follow-ups filed from that insight: a **lost-and-found** orphan detector + recovery surface (#1143) and **structural writes that confirm their result rather than returning silently** (#1144, whose framing the orphaned spec — 4,000 chars of design that nearly vanished — literally supplied: *"silently reinterpreting the user's structure is worse than refusing the operation"*). **Hardening — the reliability floor under the AI-era build.** Not a feature cut; a patch that closes a real data-loss race and adds a self-healing net, driven end to end by the dogfooding loop (hit in use → diagnosed live over MCP → fixed → Codex-hardened → recovered over MCP). Banked: **an intent replayed on conflict must reconcile, never overwrite** an array (verbatim replace is last-writer-wins that eats concurrent structural moves); **self-heal belongs at the ingest boundary, not one caller**; a whole-doc concurrency model wants a **single-parent `childIds` invariant** as a standing guarantee; the bug **orphans rather than deletes**, so recovery must not depend on knowing a UUID; and — a process scar — `Write` silently clobbers an existing test file, caught only because the suite count *dropped* (`git status` showed `M`, not `??`). **Next:** the orphan lost-and-found (#1143), confirm-not-silent structural writes (#1144), and the still-parked `#not-stalled`-vs-SOMEDAY verdict.

## v3.5.0 — 2026-08-17

*(the SOMEDAY sprint — one new status primitive + two capture-ergonomics tweaks + the deferred deps batch, plus a pre-cut review that caught a suppression gap the design table had missed; 3 feature PRs + a review-fix + a deps tail; minor)* **The status model gets its last primitive — a place for what you haven't decided to do.** After three cuts inside the 3.x AI arc, 3.5.0 steps back to the **core GTD model** and completes it: **SOMEDAY** (#1131) — a commitment marker, not a timing one. Backlog says *later* and a due date says *when*; SOMEDAY says *"I don't want to forget this, but I haven't decided to do it"* — and the semantics are the interesting part: a someday node **and its whole subtree** drop out of every day-to-day surface to live in a dedicated **Someday view** visited on purpose, where **emptying the list is a success, not a chore**. The subtree-suppression reused the exact shape of the existing archived-subtree walk — generalized into one `subtreeIdsWhere` resolver rather than a second open-coded traversal — with a deliberate **roots-vs-descendants** split (`somedayRoots` for the review view, `somedaySuppressedIds` for exclusion, ARCHIVED winning over SOMEDAY) and a documented **category-error boundary** against `#not-stalled`: SOMEDAY is a *subtree* claim ("none of this"), `#not-stalled` a *one-node* claim ("don't ask **this** node for a next action"), so they share the walk but pointedly not the scope. Around the headline, two capture-ergonomics wins straight from dogfooding: new project actions now default to **Next** (#1132 — a per-user setting, since capturing usually means intent, reversing the old #210 Backlog default), and the workbench add-action row became a **sticky, chat-style input** that scroll-anchors the item you just added as the list grows (#1133). The **deferred Dependabot batch** from 3.4.0 also landed (#1127/#1130 — framer-motion 12→13 verified, the flaky crypto canary lengthened, 0 audit vulns). The **pre-cut review gate paid off on exactly its class — and notably it was the *inline* pass, not Codex, that caught it**: SOMEDAY suppression had been wired into the five surfaces the design *table* enumerated (Next / Backlog / context / stalled / gone-quiet) but **not** the three that only happened to exclude *archived* — Due, Calendar/Agenda, Blocked — so a dated or blocked someday item still nagged from the time surfaces, contradicting the design's own "**all** day-to-day surfaces" prose and the type comment ("reachable only via the Someday view, `list_someday`, and search"). Fixed and tested before the tag (#1137). **Converging — on the model, not the AI.** A deliberate pause from the connector arc to finish the status vocabulary: SOMEDAY is the primitive the GTD model had been missing, and shipping it immediately surfaced a live product question the author is now dogfooding — whether SOMEDAY *subsumes* `#not-stalled` entirely (a `#not-stalled` tag rubs off onto descendants and shows an inherited chip the review lens correctly doesn't honor; parked pending real use rather than reversed on a whiteboard). Banked: **a status that suppresses a subtree must be audited against every read surface that already excludes its sibling** (archived), not just the ones a design table lists — an exclusion is a cross-cutting contract, and the inline "trace the read paths, not just the mutations" lens is what catches the surfaces the doc forgot; `subtreeIdsWhere` is now the single home for subtree-status suppression; and `npm run i18n:check` is a CI gate *outside* the local typecheck/lint/test/build four, so any `domain.*` key change needs `i18n:export-domain` committed (it failed #1131's CI twice before this was internalized). **Next:** the `#not-stalled`-vs-SOMEDAY verdict once the author has lived with it, the still-open MCP read-lens correctness cluster (#1079 / #1080 / #1091 / #1072), ChatGPT as a second connector (#1102), and the beyond-parity `seed_project`.

## v3.4.0 — 2026-08-16

*(three write-dogfooding PRs — write becomes the default, the AI learns to schedule, an inbox papercut — plus a two-finding Codex gate and a backlog-cleanup pass; minor)* **Write becomes the default, and the AI learns to schedule — driven by planning real life in NAM.** 3.3.0 proved a Claude-in-the-loop is a dogfooding *engine*; 3.4.0 is the first cut where that loop ran on **real personal work** — planning a family vacation and a season of Brann home matches directly over the connector — and every issue this sprint fell out of that use. **Write shed its gate** (#1116): the opt-in "allow changes" consent checkbox is retired (the sole owner always ticked it, so it was pure friction), and the structural win came free — registering the write tools **unconditionally** (a read-only token gets a clear refusal, not a missing tool) makes the advertised tool list **stable across deploy/reconnect**, dissolving the `canWrite` **flap** that had made a connection go dark for writes after the 3.2 deploy (the "can't tell *not-permitted* from *not-there*" family, now closed on the server side). With write frictionless, the friction moved to **reach**: the assistant could create work but not *schedule* it — so **`set_due`** (#1121) sets a node's due date, clock time, and range, and `add_action` / `add_next_action` can schedule at birth, so AI-created commitments finally land on the calendar / Due / agenda (the match kickoff times were the concrete miss). A capture papercut rounded it out: **`c` now quick-captures** inside the inbox process deck (#1119), mirroring the global shortcut the modal suppresses, so a thought arriving mid-triage goes to the inbox without breaking flow. The **Codex gate earned its keep on exactly the class it always does** — my inline pass was clean, and Codex found two correctness edges in the auth/scheduling code: a same-day range whose end time preceded its start that `set_due` would have persisted (a state the SPA's own due editor blocks — the lesson: **an MCP tool mirroring a domain intent must mirror the SPA's *validation*, not just the intent**, or it becomes a laxer door to the same document), and a `nam.write`-only refresh token that would exchange yet couldn't call `/mcp` (write implies read). Both hardened before the tag. Also a **backlog-hygiene pass**: three issues that shipped work had quietly resolved (#1099 write-discoverability → #1116; #908 Focus black-page → #1108; #1097 deploy-drift → serverVersion + the stable list) were closed, and the write epic (#1069) trimmed to its one leftover (`seed_project`). **Converging — the AI arc maturing from capability to fit.** 3.2 shipped write *guarded*; 3.4 makes it the *default* and starts closing the reach gaps real use exposes — the co-pilot isn't demoing the tool anymore, it's *using* it, and its friction is now the roadmap. Banked: write-by-default is validated and the owner strongly prefers it (don't re-gate without multi-user/sharing); an MCP tool list must be stable to avoid the flap (**advertised ≠ granted**); mirror the SPA's validation when exposing an intent as a tool; and shipped work leaves issues open — a periodic close-the-done pass keeps the pile honest. **Next:** the deferred Dependabot batch, the parked #1117 (get_node blocker, awaiting a live datapoint), the MCP read-lens correctness cluster (#1079/#1080/#1091/#1072), ChatGPT as a second connector (#1102), and the beyond-parity `seed_project`.

## v3.3.0 — 2026-08-13

*(the first sprint dogfooded with an AI in the loop — descriptions made reachable on both the tool and the web, one redundant path retired, one blank-page bug fixed; 4 work PRs + a pre-cut Codex-P3 hardening; minor)* **Descriptions come out of hiding — and the dogfooding loop gains a co-pilot.** 3.1.0 proved a read-only AI-over-MCP is also a *dogfooding engine*, and 3.2.0 gave it the pen; 3.3.0 is the first sprint where that loop **ran with an assistant actually in it** — the author dogfooded NAM *alongside* a Claude connected over the live MCP server, and the gripes that surfaced clustered so tightly they named their own theme: **a description should be reachable, not buried.** The cluster spans two surfaces with — deliberately — **no shared "layer down" fix** (one's a tool, one's UI, the same underlying need): on the MCP side, `get_node(id)` now returns a node's **full description text** plus its blocked-by dependencies and resources, so an assistant reviews *intent* instead of guessing from titles (#1106) — and the list-projection flag `hasNote` became **`hasDescription`** to align the read surface with the write param and the web; on the web, a project's description now **previews on the collapsed Details panel** (#1109), a two-line clamp with the action-rows' click-to-read `DescriptionPeek` popover for a long one, so you stop opening Details just to see what a project is *about*. Around the reach cluster, a deliberate **subtraction**: **context (tag-filter) bookmarking retired** (#1107) — it had overlapped with saved views since the two were invented alongside each other, so bookmarking is now for **projects (a location) only**; the removal is a ~530-line net-deletion refactor through the model, mutations, the Contexts sidebar, and `TagFilterPanel`, made **legacy-tolerant at a single read boundary** (`bookmarksOf` filters stored `tagFilter` rows out; they're ignored, never migrated or dropped — `reorderBookmarks` re-appends unmentioned ids so nothing's lost). And a genuine bug: **Focus no longer blanks** in a tab left open across a deploy (#1108) — Focus is the *only* code-split route, so a stale hashed chunk 404'd its lazy import and, with no error boundary, blanked the whole route until a manual refresh; a `ChunkErrorBoundary` now recovers (auto-reload once, loop-guarded, with a manual Reload fallback), and Focus — which lives *outside* the shell's loading gate — grew its own loading state so it stops flashing "All clear" over a still-null document. The **dual review** held to form: my inline pass was clean, and **Codex found one P3** of exactly its recurring class — the loop guard reads `sessionStorage` *inside* `componentDidCatch`, and `getItem`/`setItem` don't just fail to resolve in private-mode, they **throw**, which would break the error boundary *while it's already handling the chunk error*; fixed pre-cut by wrapping the guard in try/catch (any storage failure → skip the auto-reload, show the manual fallback) with a regression test. The **AI-auto-sprint method matured**: four issues agreed up front, worked straight through as **four independent branches → PRs** with `/demo` CF links, merged **none until the author tested in prod**, then the cut — and the merge train re-taught an old lesson, that four PRs all inserting under the same `## [Unreleased]` anchor conflict predictably, where the repo's own **`no-conflict-markers` test earned its keep** by catching a stray leftover conflict marker I'd left mid-reconcile. **Polishing — and consolidating.** No new front: this is the 3.x AI arc *refining itself*, the assistant-in-the-loop turning its own reading-friction into shipped reach on both sides of the connection, plus the kind of subtraction (kill the redundant way) and blank-page fix that make the tool feel finished. Banked: the **AI-co-pilot dogfooding produces coherent clusters**, not scattered gripes (one need → tool + UI, no forced shared abstraction); a **removed model kind stays safe by filtering at the one read accessor** and leaving stored rows untouched (ignored ≠ migrated ≠ dropped); an **error boundary's own defensive path can throw** (storage methods, not just resolution) and must be wrapped; and the `no-conflict-markers` suite is the backstop that makes a same-anchor CHANGELOG merge-train safe. Next: the MCP chat test "looks good," so the AI arc keeps compounding — **ChatGPT as a second connector** (#1102), Phase 2's one-approval **bulk `seed_project`**, the still-open review-lens correctness cluster from 3.1/3.2 (grace-period #1079, `statusChangedAt` #1080, blocked≠stalled #1072), and the write-permission-visibility posture call (#1099).

## v3.2.0 — 2026-08-13

*(the write chapter — opt-in write + always-ask guards + delete safety + a self-describing surface; 4 work PRs, ~240 lines, all `mcp/`; minor)* **The AI gains write — small in code, a milestone in capability.** 3.1.0 let an assistant *read* the workspace; 3.2.0 lets it *change* it — the second chapter of the 3.x AI arc, and the line-count belies it: the 16 write tools already existed and were scope-gated, so the work was **the gate, the guards, and the discoverability**, not the writes. **Opt-in consent** (#1090): read-only stays the default; a single sign-in checkbox is the *only* thing that grants `nam.write` — never the client's requested scope, so a connector can't escalate itself (the #1050 posture holds, now with an explicit owner opt-in), and every write is **annotated** so the client prompts before it runs. The disproportionate care went to the one **irreversible** operation, `delete_node` (#1094): it refuses to remove a node with children unless you confirm `recursive: true` (naming the blast radius — direct children + total descendants), returns a **manifest** of exactly what it removed, and — this is where the **second-vendor review earned its keep again** — aborts on a concurrent edit rather than replaying the delete onto a document it never gated. That last was a **P1 the black-box MCP reviewer couldn't have seen** (the response looked correct); Codex found it, and the fix generalised into a `replayOnConflict` opt-out on the *shared* commit engine (default-on everywhere; off only for delete, so a `synced` delete provably hit the state it gated). And because a read-only connection used to make the write tools simply *absent* — a `create_project` failing as "tool not found," indistinguishable from a typo or a stale-after-deploy cache — `get_workspace_context` now reports **`canWrite`** and a per-deploy **`serverVersion`** (#1100), so an agent learns its permissions and the server build on its first read instead of by a write that fails opaquely. The through-line is the **dogfooding loop maturing into the primary driver**: read-only was validated as a GTD reviewer in 3.1.0, and write was validated the same way — a live acceptance test drove the whole lifecycle (create → add → status → recursive delete) and confirmed the gate holds, including the subtle "a leaf must *not* need the flag, or agents pass it always and the guard evaporates." The live reviewer also surfaced the release-relevant process finding — a `make mcp-deploy` that changes the tool list leaves a **connected client holding a stale schema** with no version signal (#1097, part of why `serverVersion` shipped) — and named its own **blind spot** (black-box review can't see internal replay/consistency), now banked as calibration: weight the MCP reviewer for surface/UX, Codex for internal logic. Process scar tissue too: a macOS **case-collision** silently dropped a Makefile change from a commit (tracked lowercase `makefile`, edited as `Makefile`; #1087) — verify committed *content*, not the disk file. **Converging, deliberately narrow** — the write half of the AI arc's core, done as a tight, guarded, twice-Codex'd increment rather than a sprawl, with the engineering weight placed exactly where irreversibility lives. **Next: the post-cut sprint the dogfooding already filled** — the review-lens correctness cluster (grace-period #1079, `statusChangedAt` #1080, rolled-up `lastActivityAt` #1091), blocked≠stalled + a blocked-state projection (#1072), the write-permission-visibility posture call (#1099 option 2), ChatGPT as a second connector (#1102), and Phase 2's one-approval bulk `seed_project` (the AI drafts a tree, you say go, one write).

## v3.1.0 — 2026-08-10

*(the AI era's first landing — the remote MCP server hardened, hosted, and made review-useful; ~13 work PRs across security + deploy + read-surface + tooling; minor)* **The AI era arrives — NAM is now readable by an AI assistant.** 3.0.0 marked what NAM had *become* (the tool that replaced Trello); 3.1.0 opens what it turns toward next. The remote MCP server — the AI-on-NAM POC proven back in NamDesktop's chat→"make it in NAM"→execute flow, reimplemented here across 2.x — went from *built-but-unhosted* to **live in production**: sign in with your NamWeb account from a Claude or ChatGPT connector and the assistant reads your real workspace. The bulk of the work was **not the feature but the door**: a **pre-deploy dual security review** (Claude inline + Codex, independent) of the hand-rolled OAuth 2.1/PKCE server found **1 Critical + 5 High** (a `NAM_MCP_DEV_NOAUTH` prod fail-open; refresh-scope escalation; `trust proxy` IP-spoof; plaintext tokens/sessions at rest; open-DCR consent phishing; unenforced read scope), all closed across five workstreams (#1056–#1060). The standout was the **grant/refresh token lifecycle**, which only converged under **four rounds of adversarial Codex re-review**, each exposing the next layer of a subtle state machine: narrowed-scope-not-enforced (P1) → concurrent double-refresh (claim-first CAS) → winner-path failure-atomicity (rollback) → a benign *duplicate* refresh mid-flight revoking the whole family (a per-grant in-progress **lease**, generation advancing only after a replacement token is durably issued) → a stale lease-winner clobbering a newer claimant (an **owner-nonce** on the lease). An OAuth token lifecycle is exactly the read/write-consistency-under-concurrency class where a second vendor earns its keep, and four rounds of it is the sharpest evidence yet: the suite stayed green throughout — only adversarial review found these. Then the **deploy** (Fly.io, one always-on machine, the token store reusing the same Supabase Postgres via the Session pooler) — a productionize-not-feature sprint whose friction was all config: a Supabase password *reset that was generated-but-not-applied* triggered a long `28P01` chase (diagnosed by testing the credential *outside* Fly with a throwaway `pg` script), and the `.env`-points-at-the-*local*-stack trap sent the wrong publishable key up. Once live, the read-only connection did something better than planned — it became a **GTD reviewer**, and dogfooding *that* generated its own requirements: the read tools returned `{id,title,status,tags}`, too thin for time-based review, so **#1076** enriched every node projection (timestamps, due, ancestor **path**, node type, tag-lane classification, note-presence) and added server-side stalled/gone-quiet lenses + a one-call subtree read. The loop closed on itself — a **closed-loop handover** had the reviewer acceptance-test its *own* findings, which immediately surfaced a real dated commitment 23 days out with no next action (structurally invisible an hour before) and **quantified the nagging risk**: `list_stalled_projects` returned 38, ~30% false positives — turning the deferred intent/marker design (#1072) into a *measured* gate on shipping the in-app Review lens. Codex's scoped pass on #1076 found one P3 (the subtree cycle-guard was incomplete because `nodeView`→`buildPath` had no visited guard; hardened the shared primitive, #1084/#1083). A tooling thread rode along: the **Makefile reoriented as the human's task surface** (`make mcp-deploy`, gated — the credentialed/prod one-liners the agent can't run), which itself exposed a macOS **case-collision** that had silently dropped the file's content from the commit (tracked lowercase `makefile`, edited as `Makefile`; #1087). **Converging, decisively — a whole new front, not polish.** Where 2.x was the author hardening NAM against his own life and 3.0.0 named that, 3.1.0 is the first landing of the **3.x AI arc**: the assistant can now *look* (write is the next chapter, specced in #1069 and deliberately deferred to dogfood read-only first). Banked, richly: adversarial second-vendor review is non-negotiable on an auth state machine (four rounds); the Session-pooler deploy + not-yet-applied-password gotchas; the Makefile-as-human-surface principle and its case-collision trap; and the discovery that a **read-only AI-over-MCP is both a product in its own right and a dogfooding engine** — the web-Claude reviewer is a de-facto spec for the in-app Review lens, and its two-stream signal (content critique vs product friction) is now a standing harness. Next: the AI-review follow-ups it surfaced (#1079 stalled grace-period, #1080 `statusChangedAt`, #1072 intent/marker as the ship-gate), a Life-tree review pass, then **write-enablement (#1069)** — the AI gets the pen.
