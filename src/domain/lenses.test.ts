...
describe('unusedTags (#939)', () => {
  ...
  it('does not flag a tag that a saved view uses (#939 review, P2)', () => {
    const doc = workspace([], (d) => {
      d.registeredTags = ['weekly', 'errand'];
      // 'weekly' backs a saved view — deleting it would orphan that saved surface, so it's not
      // "unused". Only 'errand' is truly unused.
