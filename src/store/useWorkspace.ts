...
export interface UseWorkspace {
   ...
   *  an event id in a resource's `drainLedger` here means the ledger append reached the backend, so
   *  the event is safe to delete. An optimistic-only append (a failed write) is NOT here, so its
   *  event is left claimed for the next drain to re-process idempotently. */
